    ...
    def render_sidebar(self):
        ...
        with st.sidebar:
            ...
            if hasattr(self.diffusion_processor, 'get_info'):
                ...
                if device == 'cloud':
                    st.success("🌐 Procesamiento en la nube - HuggingFace API")
                    api_configured = info.get('api_key_configured', False)
                    if api_configured:
                        st.info("✅ API Key configurada correctamente")
                    else:
                        st.warning("⚠️ API Key no configurada - Configura HUGGINGFACE_API_KEY")
                elif device == 'cuda':
