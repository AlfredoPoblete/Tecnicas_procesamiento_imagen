"""
Aplicación de Edición Generativa de Imágenes - VERSIÓN SIN EMOJIS PARA WINDOWS
...
Corrección para compatibilidad con Windows (sin emojis)
"""
...
try:
    ...
    print("[OK] .env cargado con python-dotenv (si existe)")
except Exception as e:
    # No fallamos la ejecución si python-dotenv no está instalado
    print(f"[WARNING] No se pudo cargar python-dotenv: {e}")
...
class ImageEditingApp:
    ...
    def __init__(self):
        ...
        print("[TARGET] ImageEditingApp inicializado con carga lazy")
    ...
    def _ensure_diffusion_processor(self):
        ...
        if self.diffusion_processor is None:
            print("[LOAD] Cargando DiffusionProcessor bajo demanda...")
            with st.spinner("[LOADING] Cargando modelo de difusión..."):
                self.diffusion_processor = DiffusionProcessor()
            print("[OK] DiffusionProcessor cargado exitosamente")
    
    def _ensure_analyzer(self):
        """Asegurar que el GeminiAnalyzer esté cargado"""
        if self.analyzer is None:
            print("[LOAD] Cargando GeminiAnalyzer bajo demanda...")
            with st.spinner("[LOADING] Cargando analizador..."):
                self.analyzer = GeminiAnalyzer()
            print("[OK] GeminiAnalyzer cargado exitosamente")
    
    def _ensure_ui_helper(self):
        """Asegurar que el UIHelper esté cargado"""
        if self.ui_helper is None:
            print("[LOAD] Cargando UIHelper bajo demanda...")
            self.ui_helper = UIHelper()
            print("[OK] UIHelper cargado exitosamente")
    ...
    def process_image(self, image: Image.Image, method: str, **kwargs) -> Tuple[Optional[Image.Image], Dict[str, Any]]:
        ...
        try:
            ...
            method_descriptions = {
                "inpainting": "[TARGET] Inpainting: Eliminando y rellenando objetos...",
                "outpainting": "[TARGET] Outpainting: Extendiendo imagen...",
                "style_transfer": "[TARGET] Style Transfer: Aplicando estilo artístico...",
                "object_removal": "[TARGET] Object Removal: Eliminando objetos...",
                "background_replacement": "[TARGET] Background: Cambiando fondo...",
                "intelligent_composition": "[TARGET] Composición: Creando composición inteligente..."
            }
            
            description = method_descriptions.get(method, f'[TARGET] Procesando imagen con {method}...')
            ...
            with st.spinner(description):
                ...
                if hasattr(self, 'use_hf_api') and self.use_hf_api:
                    st.info("[API] Usando API de Hugging Face (sin carga de modelos locales)")
                
                # Mostrar configuración optimizada
                if method in ['inpainting', 'outpainting']:
                    steps = kwargs.get('num_inference_steps', 20)
                    st.info(f"[OPTIMIZADO] Modo optimizado: {steps} pasos (reducido para velocidad)")
                ...
                if result and metadata:
                    ...
                    st.success(f"[OK] Procesamiento completado en {processing_time}")
                    if 'api_processed' in metadata:
                        st.info("[API] Procesado vía API - Sin consumo de memoria local")
                    elif device == 'cuda':
                        st.info("[GPU] Acelerado por GPU")
                    else:
                        st.info("[CPU] Procesado en CPU - Modo de bajo consumo")
    ...
    def analyze_image(self, image: Image.Image, analysis_type: str = "comparison_analysis") -> Dict[str, Any]:
        ...
        try:
            ...
            if not api_key:
                st.error("[ERROR] No se encontró API key de Gemini configurada")
            ...
            with st.spinner('[AI] Analizando imagen'):
    ...
    def render_sidebar(self):
        ...
        with st.sidebar:
            st.markdown('<h2 style="color: white;">[INFO] Instrucciones de Uso</h2>', unsafe_allow_html=True)
            ...
            api_key = os.getenv('GOOGLE_API_KEY')
            if api_key:
                st.success("[OK] API Key de Gemini configurada")
            else:
                st.error("[ERROR] API Key de Gemini no configurada en .env")
            
            st.markdown('<hr style="border-color: #7E57C2; margin: 2rem 0;">', unsafe_allow_html=True)
            
            # Información de rendimiento OPTIMIZADA para Streamlit
            st.markdown('<h3 style="color: white;">[PERFORMANCE] Estado del Sistema Optimizado</h3>', unsafe_allow_html=True)
            
            # Verificar modo de API
            if hasattr(self, 'use_hf_api') and self.use_hf_api:
                st.success("[API] API de Hugging Face habilitada - Modelos ligeros")
                st.info("[API] Procesamiento vía cloud - Sin carga local")
            else:
                ...
                if self.diffusion_processor is not None:
                    ...
                    if device == 'cuda':
                        st.success("[GPU] GPU disponible - Procesamiento acelerado")
                    else:
                        st.warning("[CPU] CPU solamente - Modo de bajo consumo activado")
                    
                    lazy_loading = info.get('lazy_loading_enabled', False)
                    if lazy_loading:
                        st.info("[OPTIMIZADO] Carga lazy habilitada - Inicialización rápida")
                else:
                    st.info("[LOAD] Modelos locales - Se cargarán bajo demanda")
            
            # Mostrar optimizaciones específicas para Streamlit
            st.success("[STREAMLIT] Modo optimizado para Streamlit habilitado")
            st.info(f"[RESOLUCION] Resolución máxima: 256px (reducida para velocidad)")
            st.info(f"[PARAMETROS] Parámetros optimizados: pasos reducidos")
            
            if 'optimization_enabled' in st.session_state:
                st.success("[OK] Optimizaciones Streamlit: ACTIVAS")
            
            st.markdown('<hr style="border-color: #7E57C2; margin: 2rem 0;">', unsafe_allow_html=True)
            
            # Tecnologías utilizadas
            st.markdown('<h3 style="color: white;">[TECH] Tecnologías Utilizadas</h3>', unsafe_allow_html=True)
    ...
    def render_header(self):
        ...
        st.markdown(
            '<h1 class="main-header fade-in">[GENERICO] Edición Generativa de Imágenes</h1>',
            unsafe_allow_html=True
        )
            ...
            <div style="text-align: center; margin-bottom: 2rem;">
                ...
                <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(250px, 1fr)); gap: 1rem; max-width: 800px; margin: 0 auto;">
                    <div style="background: var(--card-bg); padding: 1rem; border-radius: 8px; border: 1px solid var(--card-border);">
                        <div style="color: var(--accent-purple); font-size: 1.5rem;">[IMAGEN]</div>
                        <strong>Inpainting:</strong> Eliminar y rellenar objetos de forma inteligente
                    </div>
                    <div style="background: var(--card-bg); padding: 1rem; border-radius: 8px; border: 1px solid var(--card-border);">
                        <div style="color: var(--accent-purple); font-size: 1.5rem;">[EXTENDER]</div>
                        <strong>Outpainting:</strong> Extender imágenes más allá de sus bordes
                    </div>
                    <div style="background: var(--card-bg); padding: 1rem; border-radius: 8px; border: 1px solid var(--card-border);">
                        <div style="color: var(--accent-purple); font-size: 1.5rem;">[ESTILO]</div>
                        <strong>Style Transfer:</strong> Transferir estilos artísticos
                    </div>
                    <div style="background: var(--card-bg); padding: 1rem; border-radius: 8px; border: 1px solid var(--card-border);">
                        <div style="color: var(--accent-purple); font-size: 1.5rem;">[ELIMINAR]</div>
                        <strong>Object Removal:</strong> Eliminar objetos no deseados
                    </div>
                    <div style="background: var(--card-bg); padding: 1rem; border-radius: 8px; border: 1px solid var(--card-border);">
                        <div style="color: var(--accent-purple); font-size: 1.5rem;">[FONDO]</div>
                        <strong>Background Replacement:</strong> Cambiar fondos manteniendo sujetos
                    </div>
                    <div style="background: var(--card-bg); padding: 1rem; border-radius: 8px; border: 1px solid var(--card-border);">
                        <div style="color: var(--accent-purple); font-size: 1.5rem;">[AI]</div>
    ...
    def render_upload_section(self):
        ...
        with col1:
            st.markdown('<h3 class="section-header">[UPLOAD] Carga de Imagen</h3>', unsafe_allow_html=True)
            ...
            if uploaded_file:
                ...
                if image:
                    ...
                    st.success("[OK] Imagen cargada exitosamente")
            
            # Galería de ejemplos debajo del uploader
            examples_dir = os.path.join('assets', 'ejemplos')
            if os.path.isdir(examples_dir):
                st.markdown("<div style='margin-top:1rem'><strong>[EJEMPLOS] Ejemplos rápidos:</strong></div>", unsafe_allow_html=True)
                        ...
                        with col:
                            try:
                                ...
                                if st.button(f"Usar {fname}", key=f"example_{idx}"):
                                    ...
                                    st.success(f"[OK] Ejemplo '{fname}' seleccionado")
                            except Exception as e:
                                st.error(f"Error leyendo {fname}: {e}")
        
        with col2:
            st.markdown('<h3 class="section-header">[INFO] Información de la imagen</h3>', unsafe_allow_html=True)
            ...
            if 'original_image' in st.session_state:
                ...
                st.markdown(
                    ...
                    <div style="background: rgba(187, 134, 252, 0.1); padding: 1rem; border-radius: 8px; margin-top: 1rem;">
                        <strong>[DIMENSIONES] Información de la imagen:</strong><br>
    ...
    def render_processing_section(self):
        ...
        st.markdown('<h3 class="section-header">[PROCESSING] Procesamiento</h3>', unsafe_allow_html=True)
        ...
        with col1:
            ...
            if st.button("[START] Procesar Imagen", key="process_button", type="primary"):
                # Mapear métodos
                method_mapping = {
                ...
                if result:
                    ...
                    st.success("[OK] Imagen procesada exitosamente")
                    
                    # Mostrar métricas si están disponibles
                    if 'processing_time' in metadata:
                        st.info(f"[TIEMPO] Tiempo de procesamiento: {metadata['processing_time']}")
    ...
    def render_comparison_section(self):
        ...
        st.markdown('<h3 class="section-header">[COMPARISON] Comparación Original vs Procesada</h3>', unsafe_allow_html=True)
    ...
    def render_analysis_section(self):
        ...
        with col1:
            if st.button("[ANALYZE] Análisis de resultados", key="analysis_button", type="primary"):
                # Verificar API key antes de proceder
                api_key = os.getenv('GOOGLE_API_KEY')
                if not api_key:
                    st.error("[ERROR] API Key de Gemini no configurada en archivo .env")
                else:
                    try:
                        # Obtener imágenes
                        ...
                        if not original or not processed:
                            st.error("[ERROR] No hay imágenes disponibles para analizar")
                            return
                        ...
                        with st.spinner('[AI] Analizando con Gemini 2.0'):
                            try:
                                ...
                                if result and result.get('success'):
                                    ...
                                    st.success("[OK] Análisis completado")
                                else:
                                    st.error("[ERROR] La respuesta de Gemini no fue válida")
                                    
                            except Exception as e:
                                print(f"Error con analyze_comparison_with_genai_sdk: {e}")
                                import traceback
                                traceback.print_exc()
                                st.error(f"[ERROR] Error al conectar con Gemini: {str(e)}")

                    except Exception as e:
                        import traceback
                        print(traceback.format_exc())
                        st.error(f"[ERROR] Excepción durante el análisis: {str(e)}")
        ...
        if 'processed_analysis' in st.session_state:
            ...
            st.download_button(
                label="[DOWNLOAD] Descargar análisis",
                data=export_text,
        ...
        st.markdown(
            ...
            <div style="text-align: center; margin-top: 3rem; padding: 2rem; border-top: 1px solid var(--card-border);">
                <div style="color: var(--text-secondary);">
                    [EDICION] Procesamiento Digital de Imágenes - IFTS24<br>
                    [AUTOR] Alfredo Poblete - 2025<br>
                    [FIXED] Versión con Carga Lazy Corregida - Sin "Connection Reset"
                </div>
            </div>
            """,
            unsafe_allow_html=True
