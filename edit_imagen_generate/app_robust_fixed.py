"""
Aplicación de Edición Generativa de Imágenes - VERSIÓN ROBUSTA CON MANEJO DE EXCEPCIONES
Tema oscuro con interfaz optimizada, carga lazy de modelos y manejo robusto de errores

Procesamiento Digital de Imágenes - IFTS24
Solución robusta contra crashes, timeouts y errores de conexión
"""
...
try:
    ...
    logger.info("✅ .env cargado con python-dotenv")
except Exception as e:
    logger.warning(f"⚠️ No se pudo cargar python-dotenv: {e}")

# Importar módulos del proyecto con manejo de errores
try:
    from models.diffusion_robust import DiffusionProcessor
    from models.analysis_robust import GeminiAnalyzer
    from utils.image_utils_robust import ImageProcessor
    from utils.ui_utils_robust import UIHelper
    IMPORTS_OK = True
except ImportError as e:
    logger.error(f"Error importando módulos: {e}")
    IMPORTS_OK = False

# Decorador para manejo robusto de excepciones
def robust_execution(fallback_result=None, show_spinner=True):
    """Decorador para manejo robusto de excepciones con fallbacks"""
    def decorator(func):
        @wraps(func)
        def wrapper(*args, **kwargs):
            try:
                ...
                error_msg = f"Error temporal en {func.__name__.replace('_', ' ')}: {str(e)[:100]}..."
...
def configure_page():
    """Configurar página Streamlit con manejo robusto"""
    try:
        st.set_page_config(
            page_title="Edición Generativa de Imágenes - Robusta",
            page_icon="🎨",
            layout="wide",
        ...
        logger.error(f"Error configurando página: {e}")
        # Fallback básico
        st.title("Edición Generativa de Imágenes")

class ImageEditingAppRobust:
    """Aplicación principal con manejo robusto de excepciones"""
    
    def __init__(self):
        logger.info("🚀 Inicializando ImageEditingAppRobust")
        ...
        # Inicialización lazy con manejo de errores
        ...
        # Configurar variables de estado ligeras con manejo de errores
        try:
            ...
            st.session_state['app_robust_mode'] = True
            
            self._initialized = True
            logger.info("✅ Estado de sesión configurado")
            
        except Exception as e:
            self._init_error = str(e)
            logger.error(f"Error configurando estado de sesión: {e}")
    
    @robust_execution(fallback_result=None)
    def _ensure_diffusion_processor(self):
        """Asegurar que el DiffusionProcessor esté cargado con manejo robusto"""
        if self.diffusion_processor is None:
            logger.info("📦 Cargando DiffusionProcessor bajo demanda...")
            try:
                with st.spinner("🚀 Cargando modelo de difusión..."):
                    self.diffusion_processor = DiffusionProcessor()
                logger.info("✅ DiffusionProcessor cargado exitosamente")
                return True
            except Exception as e:
                logger.error(f"Error cargando DiffusionProcessor: {e}")
                st.error(f"❌ Error cargando modelo de difusión: {str(e)}")
                st.info("💡 Intenta reiniciar la aplicación o verifica tu conexión")
                return False
    
    @robust_execution(fallback_result=None)
    def _ensure_analyzer(self):
        """Asegurar que el GeminiAnalyzer esté cargado con manejo robusto"""
        if self.analyzer is None:
            logger.info("🧠 Cargando GeminiAnalyzer bajo demanda...")
            try:
                with st.spinner("🧠 Cargando analizador..."):
                    self.analyzer = GeminiAnalyzer()
                logger.info("✅ GeminiAnalyzer cargado exitosamente")
                return True
            except Exception as e:
                logger.error(f"Error cargando GeminiAnalyzer: {e}")
                st.error(f"❌ Error cargando analizador: {str(e)}")
                return False
    
    @robust_execution(fallback_result=None)
    def _ensure_ui_helper(self):
        """Asegurar que el UIHelper esté cargado con manejo robusto"""
        if self.ui_helper is None:
            logger.info("🎨 Cargando UIHelper bajo demanda...")
            try:
                self.ui_helper = UIHelper()
                logger.info("✅ UIHelper cargado exitosamente")
                return True
            except Exception as e:
                logger.error(f"Error cargando UIHelper: {e}")
                st.error(f"❌ Error cargando interfaz: {str(e)}")
                return False
    
    def resize_images_to_same_size(self, img1: Image.Image, img2: Image.Image, target_width: int = 400) -> Tuple[Image.Image, Image.Image]:
        """Redimensionar dos imágenes para que tengan exactamente el mismo tamaño"""
        try:
            ...
            logger.error(f"Error redimensionando imágenes: {e}")
            return img1, img2  # Fallback a imágenes originales
    
    @robust_execution(fallback_result=None)
    def load_image(self, uploaded_file) -> Optional[Image.Image]:
        """Cargar imagen desde archivo subido con manejo robusto"""
        try:
            ...
            st.info("💡 Verifica que el archivo sea una imagen válida (JPG, PNG)")
            return None
    
    @robust_execution(fallback_result=(None, {}))
    def process_image(self, image: Image.Image, method: str, **kwargs) -> Tuple[Optional[Image.Image], Dict[str, Any]]:
        """Procesar imagen con manejo robusto de excepciones"""
        try:
            ...
            if not self._ensure_diffusion_processor():
                return None, {"error": "No se pudo cargar el procesador de difusión"}
            ...
            with st.spinner(description):
                ...
                # Verificar resultado
                if result is None:
                    st.error("❌ El procesamiento no produjo resultado")
                    return None, {"error": "Procesamiento falló"}
            ...
            logger.error(f"Error en process_image: {str(e)}")
            st.error(f"Error en el procesamiento: {str(e)}")
            st.warning("💡 Tip: Si el error persiste, intenta reducir el tamaño de la imagen")
            return None, {"error": str(e)}
    
    @robust_execution(fallback_result={"error": "Análisis falló"})
    def analyze_image(self, image: Image.Image, analysis_type: str = "comparison_analysis") -> Dict[str, Any]:
        """Analizar imagen con manejo robusto de excepciones"""
        try:
            # Verificar que la imagen sea válida
            if image is None:
                st.error("❌ No hay imagen para analizar")
                return {"error": "No hay imagen"}
            
            # Verificar si hay API key configurada
            api_key = os.getenv('GOOGLE_API_KEY')
            if not api_key:
                st.warning("⚠️ API key de Gemini no configurada - Usando análisis básico")
                return {"error": "API key no configurada", "mock_analysis": True}
            
            # CARGA LAZY: Asegurar que el analyzer esté cargado
            if not self._ensure_analyzer():
                return {"error": "No se pudo cargar el analizador"}
            ...
            if not analysis or "error" in analysis:
                st.warning("⚠️ El análisis automático falló, pero la imagen se procesó correctamente")
                return analysis
            ...
            st.info("💡 El procesamiento fue exitoso, pero el análisis automático falló")
            return {"error": str(e), "partial_success": True}
    
    def render_sidebar(self):
        """Renderizar panel lateral con manejo robusto"""
        try:
            with st.sidebar:
                ...
                if api_key:
                    ...
                    st.warning("⚠️ API Key de Gemini no configurada - Análisis básico")
                
                st.markdown('<hr style="border-color: #7E57C2; margin: 2rem 0;">', unsafe_allow_html=True)
                
                # Información de rendimiento
                st.markdown('<h3 style="color: white;">⚡ Estado del Sistema Robusto</h3>', unsafe_allow_html=True)
                ...
                if hasattr(self, 'use_hf_api') and self.use_hf_api:
                    ...
                    # Solo obtener info si el procesador está cargado
                    if self.diffusion_processor is not None:
                        try:
                            ...
                            logger.error(f"Error obteniendo info del procesador: {e}")
                            st.info("💻 Modelos locales - Se cargarán bajo demanda")
                    else:
                        st.info("💻 Modelos locales - Se cargarán bajo demanda")
                
                # Mostrar optimizaciones específicas para Streamlit
                st.success("🎯 Modo robusto para Streamlit habilitado")
                st.info(f"📐 Resolución máxima: 256px (optimizada para velocidad)")
                st.info(f"⚙️ Parámetros optimizados y manejo de errores")
                
                if 'optimization_enabled' in st.session_state:
                    st.success("✅ Optimizaciones robustas: ACTIVAS")
                ...
                technologies = [
                    ...
                    ("Robustez", "Manejo de excepciones, Carga Lazy")
                ]
            ...
            logger.error(f"Error renderizando sidebar: {e}")
            st.sidebar.error("Error cargando panel lateral")
    
    def render_header(self):
        """Renderizar encabezado principal con manejo robusto"""
        try:
            st.markdown(
                '<h1 class="main-header fade-in">🎨 Edición Generativa de Imágenes Robusta</h1>',
                unsafe_allow_html=True
            )
            ...
            st.markdown(
                ...
                <div style="text-align: center; color: var(--text-secondary); font-size: 1.1rem; margin-bottom: 2rem;">
                    Plataforma robusta que integra las últimas tecnologías de IA para ofrecer capacidades
                    ...
                    y análisis inteligente con Gemini 2.0. <strong>Versión con manejo robusto de excepciones.</strong>
                </div>
                """,
                unsafe_allow_html=True
            )
            ...
            st.markdown(
                ...
                <div style="text-align: center; margin-bottom: 2rem;">
                    ...
                    <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(250px, 1fr)); gap: 1rem; max-width: 800px; margin: 0 auto;">
                        ...
                        <div style="background: var(--card-bg); padding: 1rem; border-radius: 8px; border: 1px solid var(--card-border);">
                            <div style="color: var(--accent-purple); font-size: 1.5rem;">🛡️</div>
                            <strong>Manejo Robusto:</strong> Recuperación automática de errores
                        </div>
                    </div>
                </div>
                """,
            ...
            st.error("Error cargando encabezado")
    
    def render_upload_section(self):
        """Renderizar sección de carga de imagen con manejo robusto"""
        try:
                            ...
                            for idx, fname in enumerate(example_files):
                                ...
                                with col:
                                    try:
                                        ...
                                        logger.error(f"Error cargando ejemplo {fname}: {e}")
                                        st.error(f"Error leyendo {fname}")
                except Exception as e:
            ...
            with col2:
                ...
                if 'original_image' in st.session_state:
                    try:
                        ...
                        logger.error(f"Error mostrando información de imagen: {e}")
                        st.error("Error obteniendo información de la imagen")
                        
        except Exception as e:
            logger.error(f"Error renderizando sección de carga: {e}")
            st.error("Error cargando sección de imagen")
    
    def render_processing_section(self):
        """Renderizar sección de procesamiento con manejo robusto"""
        try:
            if 'original_image' not in st.session_state:
                return
            
            st.markdown('<h3 class="section-header">🎯 Procesamiento Robusto</h3>', unsafe_allow_html=True)
            ...
            with col1:
                ...
                if not self._ensure_ui_helper():
                    st.error("❌ Error cargando interfaz")
                    return
                ...
                try:
                    ...
                    st.error("Error configurando parámetros")
                    params = {}
                ...
                with process_col2:
                    ...
                    if st.button("🧹 Limpiar", key="clear_button"):
                        keys_to_clear = ['processed_image', 'processing_metadata', 'processed_analysis']
            ...
            with col2:
                ...
                if 'original_image' in st.session_state:
                    try:
                        ...
                        logger.error(f"Error mostrando imagen original: {e}")
                        st.error("Error mostrando imagen")
                        
        except Exception as e:
            logger.error(f"Error renderizando sección de procesamiento: {e}")
            st.error("Error cargando sección de procesamiento")
    
    def render_comparison_section(self):
        """Renderizar sección de comparación con manejo robusto"""
        try:
            if 'original_image' not in st.session_state:
                return
            ...
            with col1:
                ...
                try:
                    ...
                    logger.error(f"Error mostrando imagen original: {e}")
                    st.error("Error mostrando imagen original")
            ...
            with col2:
                if 'processed_image' in st.session_state:
                    try:
                        ...
                        # Verificar que ambas imágenes sean válidas
                        if original is None or processed is None:
                            st.error("❌ Una de las imágenes no es válida")
                            return
                        ...
                        st.error("Error mostrando comparación")
                else:
                    st.markdown(
            ...
            logger.error(f"Error renderizando sección de comparación: {e}")
            st.error("Error cargando comparación")
    
    def render_analysis_section(self):
        """Renderizar sección de análisis con manejo robusto"""
        try:
            ...
            with col1:
                if st.button("🔍 Análisis de resultados", key="analysis_button", type="primary"):
                    try:
                        ...
                        # Análisis con manejo robusto
                        analysis = self.analyze_image(processed)

                        if analysis:
                            st.session_state['processed_analysis'] = analysis
                            if "error" not in analysis:
                                st.success("✅ Análisis completado")
                            else:
                                st.warning("⚠️ Análisis completado con advertencias")
                        else:
                            st.error("❌ El análisis falló completamente")

                    except Exception as e:
                        logger.error(f"Error en análisis: {e}")
                        st.error(f"❌ Error durante el análisis: {str(e)}")
                        st.info("💡 El procesamiento fue exitoso, pero el análisis falló")
            ...
            logger.error(f"Error renderizando sección de análisis: {e}")
            st.error("Error cargando análisis")
    
    def run(self):
        """Ejecutar aplicación principal con manejo robusto de errores"""
        try:
            # Configurar página
            configure_page()
            ...
            if not self._initialized:
                st.error(f"❌ Error inicializando aplicación: {self._init_error}")
                return
            ...
            st.markdown(
                ...
                <div style="text-align: center; margin-top: 3rem; padding: 2rem; border-top: 1px solid var(--card-border);">
                    <div style="color: var(--text-secondary);">
                        ...
                        🛡️ Versión Robusta con Manejo de Excepciones - Sin Crashes
                    </div>
                </div>
                """,
                unsafe_allow_html=True
            ...
            logger.error(f"Error fatal en run(): {str(e)}")
            logger.error(traceback.format_exc())
            st.error(f"❌ Error fatal en la aplicación: {str(e)}")
            st.info("🔄 Intenta recargar la página o reiniciar la aplicación")

def main():
    """Función principal con manejo robusto"""
    try:
        logger.info("🚀 Iniciando aplicación robusta...")
        app = ImageEditingAppRobust()
        app.run()
    except Exception as e:
        logger.error(f"Error fatal en main(): {str(e)}")
        logger.error(traceback.format_exc())
        st.error(f"❌ Error iniciando aplicación: {str(e)}")
